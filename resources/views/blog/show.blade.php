...
<main class="max-w-2xl mx-auto px-4 lg:px-8 pb-24">
<a href="{{ route('blog.index') }}" class="text-sm text-secondary underline">&larr; Back to blog</a>
<h1 class="font-headline-xl text-3xl sm:text-4xl font-extrabold text-on-secondary-fixed tracking-tight mt-4 mb-6">{{ $post->title }}</h1>
<div class="prose max-w-none text-on-surface-variant leading-relaxed whitespace-pre-line">{{ $post->body }}</div>

<div class="bg-on-secondary-fixed rounded-2xl p-8 text-center mt-12">
<h2 class="text-xl font-extrabold text-surface mb-3">Ready to run your shop on Tillora?</h2>
<a class="inline-flex items-center justify-center px-6 py-3 rounded-full bg-primary-container text-on-secondary-fixed font-bold" href="{{ route('signup.index') }}">Get Started</a>
</div>
</main>
